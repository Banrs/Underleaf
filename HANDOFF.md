...
|---|---|---|---|
| LaTeX editor | CodeMirror in SwiftUI's `WebView`/`WebPage` (`web/embed/editor.html`) | CodeMirror in WebView2 | CodeMirror |
...
- **Window:** one `NavigationSplitView`, sized to the macOS 27 UI kit (`~/Downloads/Apple macOS 27 UI Kit.sketch` on the owner's Mac).
  ...
  - Detail: the source beside the PDF, a build panel below (Issues | Build Log, the log an `NSTextView`), a status bar, and a trailing inspector on edge-to-edge system glass. The status bar is text, as Finder's is (HIG, Windows: a little information about the window's contents): the build's status (a button: it opens the panel on Issues), the save state, the line, the counts and the engine; then, past a hairline, the build panel's toggle. Its controls are borderless, as those in Xcode's bottom bars are: no hover bezels, and the toggle a SwiftUI `Toggle` in `.button` style that the system tints with the accent while the panel shows (the text's `.secondary` style is on the text only, since on the toggle it hid that tint). Spacing: the kit's 8 pt either side of the hairline and at the bar's ends, except where the bar meets a window corner (the leading one with the sidebar hidden, the trailing one with the inspector hidden), where it keeps clear by the system's own corner insets (16 pt): `WorkspaceView` reads `containerCornerInsets` with `onGeometryChange` into `AppModel.windowCorners`, because a view inside the split's panes sees none (checked: 16 × 16 at the root host, zero in a nested one, so `containerCornerOffset` can't work there). Tried and dropped (2026-09-26): the status item as the panel's toggle with the icon gone; the icon as a bezelled accessory toggle (a 40 × 21 bezel, too much space round the icon); 12 pt ends (too tight at the corner, uneven elsewhere).
...
- **Layout rules learnt the hard way:**
  ...
  - SwiftUI first (the owner, 2026-09-26: SwiftUI over AppKit and current APIs over older ones, everywhere). The editor is SwiftUI's `WebView` over a `WebPage` (`EditorBridge`, `EditorView`; macOS 26): the bundled web/ through a `URLSchemeHandler`, the page's messages through its configuration's `WKUserContentController`, links through a `NavigationDeciding`, focus through `.focused`, and a dead web process through `WebPage.NavigationError.webContentProcessTerminated` on `navigations` (checked by killing it twice: the page reloads and gets its file, keys, symbols and appearance back). The project window is found by its scene id ("main", `NSApp.projectWindow`), not through the editor's view. AppKit stays only where SwiftUI can't do the job: the splits (below), zoom and Share's segmented control, the bars' search fields (SwiftUI's search field goes only in a toolbar or sidebar), PDFKit's view, and the build log's text view (megabyte logs and the system find bar).
