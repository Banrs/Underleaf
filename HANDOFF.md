...
- **The source bar matches the Mac's source tools** (`web/src/sourcebar.js`): the same tools and order as the Mac's toolbar items over the source (the Mac joins bold | italic and the math into one Format group; it moved them into its window toolbar on 2026-09-26; the web keeps an in-pane bar), folding into ⋯ from the end, a location row under it, and a symbol palette whose symbols are wrapped in `$…$` outside math (`insertSymbol`, `mathModeAt` in `web/src/editor.js`). Files sit over a docked File Outline (a listbox) with a resizable divider (`outlineHeight` pref); the outline follows the top visible line.
...
- **Window:** one `NavigationSplitView`, sized to the macOS 27 UI kit (`~/Downloads/Apple macOS 27 UI Kit.sketch` on the owner's Mac).
  - Sidebar (`SidebarView.swift`): one native `List` in `.listStyle(.sidebar)` with two sections, "Files" (add menu in its header) and "Outline" (only for a `.tex` file), each folding from its header's hover chevron as Finder's, Mail's and Notes' do (`Section(isExpanded:)`, remembered in `SidebarFilesExpanded` / `SidebarOutlineExpanded`). Project search sits at the top; its results replace both sections in a list of their own (one list switching between two sections and grouped hits kept stale rows and a stray "Outline" header after the search was cleared).
    - The list's one selection is the open file. Headings are a table of contents, not a second selection: real list rows, untagged, so no capsule; flat, with no disclosure (the section folds as a whole), and indented by `Outline.depths`. The outermost headings start where the file names do (a `Label` with its icon hidden), in every project and at every sidebar size. The rows are the list's own height (the sidebar list has one row height; neither `sidebarRowSize` nor `defaultMinListRowHeight` changes it per section); the text is a step down from the file names' by the user's Sidebar icon size (`HeadingMetrics`: 13 / 11 / 10 pt for Large / Medium / Small), Regular so the current heading stands out. The current section (the top visible line, `ProjectModel.topLine`, not the caret) is in the accent colour and semibold. On purpose, the location row's section follows the caret instead, as Xcode's jump bar does: it names where typing and Section Level act, while the outline shows where you are reading, so after scrolling without clicking the two can differ. A tooltip only on a heading cut short. Clicking a heading scrolls it to the top of the source (`reveal(line, atTop)`) and leaves focus where it was.
    - Only `OutlineRows` reads `topLine`, and `HeadingRow` is equatable, so scrolling the source re-renders the outline's changed rows, not the files (1,500 headings: about 15–25% CPU while wheel-scrolling). VoiceOver reads each heading as its own row with its level. The trade-off of no selection: the arrow keys walk the files only (headings are reachable by click, VoiceOver, Full Keyboard Access and the location row's section menu). The sidebar brings the current heading into view only when the files are scrolled away or folded, never scrolling them out of view itself.
  ...
  - The window toolbar holds every pane action, as Pages', Keynote's and Xcode's do (2026-09-26, evening; `WorkspaceToolbar` in `WorkspaceView.swift`): back (Close Project, as the web's and Windows' title bars have) at the leading edge; the source's tools over the source (undo | redo, Section Level, Format (bold | italic | inline math | display math, one group as Pages' text styles), Symbols, Insert; Link / Reference / Citation, Figure / Table and the lists are in Customize Toolbar… but hidden by default); then, trailing, over the PDF: a fixed space, Compile (prominent), a fixed space (so its tinted glass doesn't tint the zoom group's rim), zoom (out | scale menu | in), a fixed space, Share, a fixed space, and the PDF and inspector toggles (Hide/Show PDF, Hide/Show Inspector). Commenting out is only in the Format menu. There are no in-pane action bars any more.
  - Under the toolbar, the source's location row (project › folders › file › section) at the system's size, as Xcode's jump bar; the source's find bar sits under it. The PDF's pages run to the top of their pane, and what acts on them floats over them on Liquid Glass (2026-09-26, night): at the foot, the page (previous / "Page 2 of 5" / next) and, when the PDF no longer matches the source, a freshness button that fixes it ("Preview Out of Date" compiles; "Last Successful Build" shows the issues); at the top, Find in PDF (field, count, previous / next, Done). The PDF's old page row, an empty strip until a PDF existed, is gone.
  - **Design rules (cleanup pass, 2026-09-26):**
    - Liquid Glass wherever it applies (the owner's direction, 2026-09-26): the window toolbar (every action is in it), the sidebar column, the inspector (edge-to-edge glass beside the content, as WWDC25 310 describes inspectors), the controls floating over the PDF, popovers, menus, sheets and alerts. Never on content: the editor, the PDF pages, the log, list rows, the empty states' buttons (`.borderedProminent`, content). The location row, the source's find bar, the build panel's header and the status bar stay flat bars stacked above their content on one opaque `BarMetrics.background` (`windowBackgroundColor`), so they read as one chrome block with the toolbar: they don't float over anything.
    ...
    - Bars: `PaneBar` (one row of a pane's own actions: the build panel's header), `PaneBarRows` (the source find bar's two rows), `SecondaryBar` (the location row and the status bar: one text style and control size for both).
    ...
    - Toolbar items are system controls the toolbar sizes and glasses itself: related buttons are one `ControlGroup` item each (one glass piece, one entry in the » menu), Section Level is a `Picker(.menu)` (checks the current level; it carries `.accessibilityLabel("Section Level")`, as the toolbar label didn't reach VoiceOver, which read the pop-up's symbol name), zoom is a `ControlGroup` of − / a `Menu` of the scale (its label padded with figure spaces to four digits, so the group keeps its width and − stays under the pointer as the scale changes; any percentage: Fit Width / Fit Height and an inline `Picker` of presets, the current one checked) / +, Insert is a pull-down of every template (with its menu chevron, as the UI kit's toolbar pull-down), Symbols is its own item so the palette popover points at it, Share is `ShareLink` (the system picker, anchored to the item), and Compile is `.borderedProminent`, which the toolbar draws as prominent (tinted) glass. `.glassProminent` looks the same in the toolbar but draws nothing in the Customize Toolbar palette, so it isn't used. While a build runs Compile becomes Stop, prominent too (a plain Stop joined its neighbours' glass in a narrow window). Adjacent plain buttons share one glass piece (AppKit's rule; the PDF and inspector toggles read as one). Adjacent `ControlGroup` items don't: their two capsules melt together with a liquid neck between them, so no two groups sit side by side by default (bold | italic and the math became one Format group; a fixed space parts zoom from Share). A customization can still put two groups together.
    - The PDF and inspector toggles are plain buttons whose title and help say what they will do, as the sidebar's toggle and Xcode's inspector button: as `Toggle`s the toolbar drew their on state as solid accent discs, so there were two or three tinted controls beside Compile, the one that should be.
    - The toolbar is customizable (`.toolbar(id: "workspace")`, View › Customize Toolbar…); the item ids are `WorkspaceToolbar.Item`'s raw values and are also the `NSToolbarItem` identifiers. Back can't be moved. Narrow windows fold items into the toolbar's own » menu from the end; Compile and the pane toggles fold last (`visibilityPriority(.high)`), so at the 960 pt minimum Insert, zoom and Share are in », and the toggles stay. Once anything folds, AppKit packs the visible items from the leading edge and the flexible space takes nothing, so the room the folded items gave up sits before », not before Compile (a flexible space kept at high priority changed nothing); the fixed space before Compile keeps it apart from the source's tools there. In » the zoom group reads "Zoom Out / 93% / Zoom In" with no icon: SwiftUI gives a group no menu form of its own, and AppKit's `menuFormRepresentation` would be reset on update, so it is left.
    - Icons only: `NSToolbar.keepIconsOnly()` (on each window update, as SwiftUI makes the toolbar after the window) turns off the display-mode choice in Customize Toolbar…. Under Icon and Text AppKit drew the labels of every `ControlGroup` member (Bold, Zoom In…) disabled while they were enabled: SwiftUI's members have no action, and AppKit's validation and SwiftUI's `isEnabled` disagreed. Turning off the members' `autovalidates` and setting `isEnabled` fixed the labels only while forced, since SwiftUI never sets it; it wasn't kept.
    - Customize Toolbar…'s sheet is as wide as the default set laid out with labels (about 1350 pt), wider than the 960 pt minimum window: AppKit widens the window to about 1000 pt and leaves it so after Done. Each group member is its own palette entry (AppKit's), which is why the palette is long.
  ...
  - The source's toolbar items follow Overleaf's toolbar (`SourceBars.swift`): undo | redo, a section-level menu for the caret's line (`setHeading`), bold | italic, inline math | display math, a symbol palette (a popover, `symbolGroups`), then Insert, a pull-down of every block, list and reference template. They are off for files that aren't `.tex` (undo and redo stay).
  ...
  - The PDF's view controller (`PDFController`) is `AppModel.pdf`, made anew for each project, so the toolbar's zoom acts on the PDF pane's view. Toolbar zoom goes to it directly; the View menu's zoom goes through `requestPDF`, which also shows the PDF.
  - Settings › General › Toolbar Size is gone with the pane bars (and its `paneBarSize` preference, removed at launch): the window toolbar is the system's size. The secondary rows and status bar are 28 pt. The status bar leaves out the save state while a build runs (it said "Compiling…" twice) and, in a narrow window, drops whole items rather than truncating them (`ViewThatFits`): the engine first (the inspector and the Compile menu show it), then the word and line counts, then the save state. Show / Hide Word Count is in the status bar's context menu and the View menu, as Pages has it, not in Settings (`AppModel.showWordCount`, the same `showWordCount` preference).
...
- **Layout rules learnt the hard way:**
  ...
  - The workspace toolbar shows no title (`.toolbar(removing: .title)`, Notes' pattern): a shown title took all the toolbar's free room ahead of any flexible space, so every item sat trailing and the source's tools ended up over the PDF. The location row names the project and file; the window keeps its title and subtitle for the Window menu and Mission Control. With the title go the title bar's proxy icon, its path menu and its "Edited" marker: `.navigationDocument` still sets the represented file (the Window menu shows it), and the status bar shows Edited / Saved. The start window keeps its "Projects" title.
  - The PDF's group sits trailing rather than following the source | PDF divider. Following it needs an `NSTrackingSeparatorToolbarItem` for the `PDFSplit` split view; SwiftUI has no item for one and owns the toolbar's delegate (`SwiftUI.ToolbarPlatformDelegate`), so adding one means swapping or subclassing that delegate at run time. Not done: SwiftUI rebuilds items on customization and update, and this window has crashed on layout loops before. (SwiftUI does add its own tracking separator, `com.apple.SwiftUI.splitViewSeparator-0`, for the sidebar column only.) So with the inspector shown Compile and zoom sit over the inspector, and in a wide sidebar-less window the source's tools can run past the divider.
  - SwiftUI resets `NSToolbarItem.visibilityPriority` on every update, so AppKit can't set it. SwiftUI's `visibilityPriority` (macOS 26.1) is declared only from the macOS 27 SDK, so it is behind `#if compiler(>=6.4)` (`keptInView`); with CI's 26.5 SDK the toolbar folds from its end, the pane toggles first. Only CI builds with that SDK; the owner's local builds (and `/Applications/TeXLocal.app`) have the 27 SDK and keep the toggles.
  - With the macOS 26 SDK a toolbar builder block takes at most ten items (the 27 SDK takes more), so `WorkspaceToolbar` builds its groups in separate `@ToolbarContentBuilder` properties. The 26.5 SDK is at `/Library/Developer/CommandLineTools/SDKs/MacOSX26.5.sdk` on the owner's Mac: `xcrun swiftc -typecheck -swift-version 6 -sdk <it> -target arm64-apple-macos26.0 -I crates/texlocal-ffi/include apps/macos/TeXLocal/*.swift` (without `-swift-version 6` it fails on `Outline.swift`'s regex literal) checks a change against it (with the `compiler(>=6.4)` gate edited off in a copy).
...
- **Tests:** 32 XCTests, including the outline tree, that a bar's group fits its 40 pt bar, that a floating glass capsule is the toolbar's 36 pt, that the file watcher tells a change written in place and one saved by renaming over the file (and keeps watching after), the window toolbar's default items, spaces and their order (the flexible and a fixed space before Compile, the fixed spaces after Compile, zoom and Share; read from the toolbar's delegate, so a saved customization doesn't change it), the build panel keeping within its largest share, the source find count, the duration text, and the Build Panel and spelling menu items. The test scheme sets `TEXLOCAL_DATA=/tmp/texlocal-xctest`, and its pre-action copies `web/src/workspace.js` there for the command-table test: the tests run inside TeXLocal.app, which would otherwise need Documents access, and macOS asks again after every re-signing build, blocking the read.
...
- **Seen, not fixed:** in full screen the toolbar's zoom label was clipped to "1…" on the critic's display; not reproduced here (full screen on a 1512 pt display, entered from 960 pt and from wider windows, and windows 1000–1240 pt wide all read the scale in full), so there is no fix to check yet. Once, an automatic build was reported failed with a log cut short mid-run while the PDF was written; the next build succeeded (not reproduced).
