...
- **Window:** one `NavigationSplitView`, sized to the macOS 27 UI kit (`~/Downloads/Apple macOS 27 UI Kit.sketch` on the owner's Mac).
  ...
  - Detail: the source beside the PDF, a build panel below (Issues | Build Log, the log an `NSTextView`), a status bar, and a trailing inspector on edge-to-edge system glass. The status bar is text, as Finder's is (HIG, Windows: a little information about the window's contents): the build's status (a button: it opens the panel on Issues), the save state, the line, the counts and the engine; then, past a hairline (`ToolSeparator`), the build panel's toggle, a SwiftUI `Toggle` in `.button` style whose bezel stays on while the panel shows, as the kit's borderless buttons ('on = bezel') are (it was a plain button tinted by hand). Its symbol is at the large image scale, which makes it the status item's height (21 pt), so the two sit level (at the bar's small scale it was 18). For a while (2026-09-26) the status item was the toggle and the icon was gone; the owner wanted the icon back. The status bar's ends are as far from its edges as its items are from each other (12 pt, `SecondaryBar.edgeInset`), so the toggle sits centred between the hairline and the window's edge, and "Compiled…" as far from the sidebar or the window's edge: the toggle's bezel clears the window's rounded corner and nests with it. SwiftUI's `containerCornerOffset` would do this from the window's own corner insets, but a view inside the split's panes sees none (checked: 16 × 16 at the root host, zero in a nested one).
