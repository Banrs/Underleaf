...
- **Window:** one `NavigationSplitView`, sized to the macOS 27 UI kit (`~/Downloads/Apple macOS 27 UI Kit.sketch` on the owner's Mac).
  ...
  - PDF bar: Compile, then the − | % | + control group, a separator, then Share. The % label is padded with figure spaces to four digits, so the group keeps its width and − stays under the pointer. No forced sizes (the owner, 2026-09-26: native small / regular / large throughout): controls take their control size's own dimensions and SF Symbols their own; no frames on icons or rows (an icon square made the bezels 26 pt; the recents rows' extra padding is gone). The bar heights aren't literals either: `BarMetrics.barHeight` and `secondaryBarHeight` are the system's bezeled-control height at the bar's control size (measured once, `controlHeight`: 24 pt regular, 20 pt small) plus the kit's margins, so every bar is one height whatever it holds and the panes' hairlines line up, as AppKit's bars do. The find capsule's 36 pt is its large search field plus 4 pt of glass, no minimum height. Share uses `NSSharingServicePicker`, anchored to its button through `ViewAnchor`: `ShareLink` opened centred on the PDF. Toolbar-free zoom (the bar's) acts on the pane directly; the View menu's zoom goes through `requestPDF`, which also shows the PDF.
