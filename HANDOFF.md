...
- **Window:** one `NavigationSplitView`, sized to the macOS 27 UI kit (`~/Downloads/Apple macOS 27 UI Kit.sketch` on the owner's Mac).
  ...
  - The window toolbar holds a back button (Close Project, as the web's and Windows' title bars have), the window's title (the file, the project as subtitle, with its proxy icon), then Share on its own glass (the system's `ShareLink` for the PDF, as Pages' and Preview's Share sits: it opens under the button; in the PDF bar it could only draw as a push button beside the segmented zoom group, and in a control group a share link does nothing), then the PDF and inspector toggles sharing one piece, as related buttons do. What acts on a pane sits over it, in the in-pane accessory bars under the toolbar (restored 2026-09-26: `36e8c93` had moved every action into a customizable window toolbar, and the owner rejected it).
  ...
  - Over the PDF: Compile (prominent; a spinner and Stop while building), zoom (one native `ControlGroup`: − | the scale as a menu, "77% ⌄" | +). Under it, the page row (below); under that, while shown, Find in PDF, a bar as the source's find bar is (field, previous / next, count, Done).
  ...
  - PDF bar: Compile, then the − | % | + control group (Share is the window toolbar's). The source bar keeps its flat accessory-bar icons (the owner rejected a TextEdit-style all-bordered bar, 2026-09-26: TextEdit isn't a current reference). The % label is the scale as it is, in monospaced digits; with its menu's chevron after it, AppKit centres the two together (padding it with figure spaces pushed three-digit scales off centre). No forced sizes (the owner, 2026-09-26: native small / regular / large throughout): controls take their control size's own dimensions and SF Symbols their own; no frames on icons or rows (an icon square made the bezels 26 pt; the recents rows' extra padding is gone). The bar heights aren't literals either: `BarMetrics.barHeight` and `secondaryBarHeight` are the system's bezeled-control height at the bar's control size (measured once, `controlHeight`: 24 pt regular, 20 pt small) plus the kit's margins, so every bar is one height whatever it holds and the panes' hairlines line up, as AppKit's bars do. The find capsule's 36 pt is its large search field plus 4 pt of glass, no minimum height. Toolbar-free zoom (the bar's) acts on the pane directly; the View menu's zoom goes through `requestPDF`, which also shows the PDF.
