...
enum Compiler {
    ...
    private static func run(_ cmd: String, _ args: [String], cwd: URL? = nil, timeout: TimeInterval = timeout) -> RunResult {
        ...
        p.waitUntilExit()
        killer.cancel()
        return RunResult(code: p.terminationStatus, stdout: o, stderr: e)
    }
    ...
    static func compile(_ root: URL, overrides: [String: Any] = [:]) throws -> CompileResult {
        ...
        guard FileManager.default.fileExists(atPath: root.appendingPathComponent(mainFile).path) else {
    ...
    static func synctexForward(_ root: URL, file: String, line: Int) throws -> SyncForward {
        ...
        let r = run("synctex", ["view", "-i", "\(line):1:./\(file)", "-o", pdf.path], cwd: root, timeout: 10)
