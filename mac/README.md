...
- **Swift → web** (drive the editor): the web pane exposes `window.TeXLocal`:
  `open(projectId, path, content, dark)`, `format(kind)`, `undo()`, `redo()`,
  `find()`, `gotoLine(n)`, `reloadPdf(url)`, `setDark(bool)`.
- **web → Swift** (report changes): `window.webkit.messageHandlers.*.postMessage`:
  - `save` → `{ project, path, content }` (autosave; the project id lets Swift
    route a save that outlived a project switch to the right root)
  - `state` → `{ project, dirty }` (drives the toolbar "Saved/Unsaved")
...
The whole `web/` tree is bundled as a **folder reference** (see `project.yml`)
and served by the scheme handler at runtime, so rebuilding the web bundle
(`npm run build`) is enough — no need to regenerate the Xcode project.
...
native toolbar · dark-mode follows the system · inverse SyncTeX (PDF → source).

**TODO:**
- project & file management (create/rename/delete), project-wide search, and
  symbol autocomplete — these live in the JS backend (`server/projects.js`);
  port them alongside the UI that surfaces them
- a richer compile-error surface (currently the window subtitle reports
  failures; the Electron app has a full log pane)
- forward SyncTeX (editor → PDF)
- zip export, PDF "Save As"
- not sandboxed (GitHub distribution); for the Mac App Store you'd need a
  security-scoped bookmark to the TeX install — see the root
  `docs/shell-and-design.md`
