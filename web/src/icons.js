// Minimal 24px stroke icons (paths adapted from Lucide, ISC license).
...
// Settings gear is the same glyph as the file-type cog — alias, don't duplicate.
PATHS.gear = PATHS.cog;

export function icon(name) {
  const span = document.createElement('span');
  span.className = 'icon';
  span.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${PATHS[name] ?? PATHS.file}</svg>`;
  return span;
}
